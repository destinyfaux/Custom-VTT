...
import { computeVisibility, buildWallGrid, isVisionBlocking } from '../utils/VisibilityEngine';
...
const resolveMapUrl = (mapInput) => {
  ...
  if (typeof mapInput === 'string') {
    ...
    rawUrl = mapInput.url || mapInput.mapUrl || mapInput.thumbnail || mapInput.path || mapInput.name || '';
  }
...
const GRID_SIZE = 70;
const PIXELS_PER_FOOT = GRID_SIZE / 5;
...
export default function CanvasMap({
  ...
  const videoElementsRef = useRef(new Map()); // stampId/url -> HTMLVideoElement
  ...
  const viewStateRef = useRef({ x: 0, y: 0, scale: 1 });
  const currentMapSrcRef = useRef(null);
  const initialFitDoneRef = useRef(false); // Prevents resetting zoom after first fit
  ...
  const isFogDirtyRef = useRef(true);
  ...
  // Visibility cache throttled to ~30 FPS
  ...
  const visibilityData = useMemo(() => {
    ...
    playerTokens.forEach(t => {
      const renderPos = netInterpolation.getPosition(t.id, t.x, t.y);
      const size = GRID_SIZE * (t.size || 1);
      const center = { x: renderPos.x + (size / 2), y: renderPos.y + (size / 2) };
      const isNight = dayNight?.mode === 'night';
      const storedVision = t.visionRadius ?? 120;
      const visionFeet = isNight ? (t.darkvisionRadius || 0) : (storedVision === 1200 ? 120 : storedVision);
      // Character senses are stored in feet; the canvas uses pixels.
      const baseRadius = visionFeet * PIXELS_PER_FOOT;
      if (baseRadius > 0) {
        const polygon = computeVisibility(center, validWalls, baseRadius, mapImage.width, mapImage.height, wallGridRef.current);
        visSources.push({ x: center.x, y: center.y, radius: baseRadius, color: null, polygon });
        playerPolygons.push(polygon);
      }
      if (t.torchActive) {
        const torchRadius = 280;
        const polygon = computeVisibility(center, validWalls, torchRadius, mapImage.width, mapImage.height, wallGridRef.current);
        // A torch is a primary player-vision source. It must reveal fog even
        // when the character has no darkvision at night.
        visSources.push({ x: center.x, y: center.y, radius: torchRadius, color: null, isTorch: true, polygon });
        playerPolygons.push(polygon);
      }
    });
    ...
    return newData;
  }, [tokens, validWalls, lights, mapImage, dayNight]);

  useEffect(() => {
    visibilityCacheRef.current.timestamp = 0;
    isFogDirtyRef.current = true;
  }, [dayNight]);

  // Build Spatial Wall Grid
  useEffect(() => {
    if (!mapImage) {
    ...
    wallGridRef.current = buildWallGrid(validWalls, w, h);
    isFogDirtyRef.current = true;
  }, [validWalls, mapImage]);
  ...
  // Synchronized Shapes Draw function
  ...
  // ─── GEOMETRIC WALL & INTERACTIVE DOOR RENDERER ───────────────────────────
  const drawWallsAndDoors = useCallback((ctx) => {
    const scale = viewStateRef.current.scale;
    const isDmEditing = role === 'DM' && (tool === 'draw' || tool === 'erase');

    walls.forEach(w => {
      const type = w.wallType || 'normal';
      const isOpen = Boolean(w.isOpen);
      const isLocked = Boolean(w.isLocked);
      ...
      // 2. WINDOW (Always visible to DM; subtle glass glint to players)
      if (type === 'window') {
        ctx.save();
        ctx.strokeStyle = '#38bdf8';
        ctx.lineWidth = 3 / scale;
        ctx.globalAlpha = isDmEditing ? 0.9 : 0.45;
        ctx.lineCap = 'round';
        ctx.beginPath();
        ctx.moveTo(w.x1, w.y1);
        ctx.lineTo(w.x2, w.y2);
        ...
        // Inner dashed line for glass look
      ...
      // 3. SECRET DOOR
      if (type === 'secret_door') {
        // Invisible to players when closed
        if (!isOpen && role !== 'DM') return;

        ctx.save();
        if (!isOpen && role === 'DM') {
          // Closed Secret Door (DM view)
          ...
          // Secret "S" Badge in center
          ctx.fillStyle = '#7e22ce';
          ctx.beginPath();
          ctx.arc(midX, midY, 9 / scale, 0, Math.PI * 2);
          ctx.fill();
          ...
          // Open Secret Door (Both DM & Players)
          ...
          // Open swing arc
          ctx.beginPath();
          ctx.arc(w.x1, w.y1, length, angle, angle + Math.PI / 2);
          ctx.stroke();
        }
        ctx.restore();
        return;
      ...
      // 4. STANDARD INTERACTIVE DOOR (Visible to everyone)
      if (type === 'door') {
        ctx.save();
        if (isOpen) {
          // Open Door: Thin dashed threshold + swung leaf
          ctx.strokeStyle = '#a76d38';
          ctx.lineWidth = 2 / scale;
          ...
          const leafEndX = w.x1 + Math.cos(swingAngle) * (length * 0.85);
          const leafEndY = w.y1 + Math.sin(swingAngle) * (length * 0.85);
          ...
          ctx.beginPath();
          ctx.arc(w.x1, w.y1, 4 / scale, 0, Math.PI * 2);
          ctx.fill();
        } else {
          // Closed Door: Solid wooden bar with brass hardware
          ...
          // Center Handle / Brass Knob
          ctx.fillStyle = isLocked ? '#ef4444' : '#f59e0b';
          ctx.beginPath();
          ctx.arc(midX, midY, 6 / scale, 0, Math.PI * 2);
          ctx.fill();
          ctx.strokeStyle = '#ffffff';
          ctx.lineWidth = 1 / scale;
          ctx.stroke();

          // Lock badge icon if locked
          if (isLocked) {
            ctx.fillStyle = '#ffffff';
            ctx.font = `bold ${8 / scale}px sans-serif`;
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ctx.fillText('🔒', midX, midY - 12 / scale);
          }
        }
        ctx.restore();
      }
  ...
  const draw = useCallback(() => {
    ...
    // Protection against subpixel canvas resize clears/flicker
    ...
    // Always read from synchronous viewStateRef
    ...
    ctx.save();
    // Apply panning and zooming transformations from ref
    ctx.translate(vs.x, vs.y);
    ctx.scale(vs.scale, vs.scale);

    // Viewport culling helpers
    const visibleX = -vs.x / vs.scale;
    const visibleY = -vs.y / vs.scale;
    const visibleW = canvas.width / vs.scale;
    const visibleH = canvas.height / vs.scale;
    ...
    if (mapImage && mapImage.complete) {
      ...
      // 2. Stamps (viewport culled with hardware decoder pause optimization)
      stamps.forEach(s => {
        const isVisibleInViewport = !(
          s.x + s.width < visibleX || s.x > visibleX + visibleW ||
          s.y + s.height < visibleY || s.y > visibleY + visibleH
        );

        // Hardware video decoder management
        const cachedVideo = videoElementsRef.current.get(s.url);
        if (cachedVideo) {
          if (!isVisibleInViewport && !cachedVideo.paused) {
            cachedVideo.pause();
          } else if (isVisibleInViewport && cachedVideo.paused) {
            cachedVideo.play().catch(() => {});
          }
        }

        if (s.hidden && role !== 'DM') return;
      ...
      // 4. Fog of War (Cached buffer rendering)
      if (!fogCanvasRef.current) {
        fogCanvasRef.current = document.createElement('canvas');
        isFogDirtyRef.current = true;
      }

      if (isFogDirtyRef.current) {
        renderOffscreenFog(fogCanvasRef.current, mapImage.width, mapImage.height, visibilityData, role);
        isFogDirtyRef.current = false;
      }
      ...
      // 6. Geometry Walls, Interactive Doors, Secret Passages & Windows
      drawWallsAndDoors(ctx);
      ...
      if (role === 'DM' && tool === 'lights') {
        lights.forEach(l => {
          ...
          // Outer glow
          ctx.beginPath();
          ctx.arc(l.x, l.y, 10 / vs.scale, 0, Math.PI * 2);
          ctx.fillStyle = l.color || '#e6b422';
          ctx.globalAlpha = 0.35;
          ctx.fill();
          // Inner dot
          ctx.beginPath();
          ctx.arc(l.x, l.y, 4 / vs.scale, 0, Math.PI * 2);
          ctx.fillStyle = l.color || '#e6b422';
          ctx.globalAlpha = 0.9;
          ctx.fill();
      ...
      if (role === 'DM' && tool === 'lights' && previewPoint) {
        ...
        ctx.setLineDash([10 / vs.scale, 5 / vs.scale]);
        ctx.stroke();
        ctx.setLineDash([]);
        // Center dot
        ctx.beginPath();
        ctx.arc(previewPoint.x, previewPoint.y, 6 / vs.scale, 0, Math.PI * 2);
      ...
      if (draggedToken && dragOrigin) {
        ...
        if (t) {
          ...
          const endX = renderPos.x + size / 2;
          const endY = renderPos.y + size / 2;

          const dx = Math.abs(renderPos.x - dragOrigin.x);
          const dy = Math.abs(renderPos.y - dragOrigin.y);
          ...
          ctx.font = `bold ${14 / vs.scale}px sans-serif`;
          const textWidth = ctx.measureText(label).width;
          
          ctx.fillStyle = "rgba(0,0,0,0.85)";
          ctx.fillRect(midX - (textWidth/2 + 5), midY - 12 / vs.scale, textWidth + 10, 24 / vs.scale);
          ctx.strokeStyle = "#e6b422";
          ctx.lineWidth = 1 / vs.scale;
          ctx.strokeRect(midX - (textWidth/2 + 5), midY - 12 / vs.scale, textWidth + 10, 24 / vs.scale);
          
          ctx.fillStyle = "#e6b422";
          ctx.textAlign = "center";
          ctx.fillText(label, midX, midY + 5 / vs.scale);
        }
      }
      ...
      pings.forEach(ping => {
        ...
        const ringRadius = (30 + progress * 60) / vs.scale;
        ...
        ctx.beginPath();
        ctx.arc(0, 0, 6 / vs.scale, 0, Math.PI * 2);
        ...
        ctx.shadowBlur = 4;
        ctx.globalAlpha = 1;
        ctx.fillText(ping.playerName || '?', 12 / vs.scale, -12 / vs.scale);
      ...
      placedTokens.forEach(t => {
        ...
        if (role !== 'DM' && t.type === 'npc') {
          const size = GRID_SIZE * (t.size || 1);
          const centerX = renderPos.x + (size / 2);
          const centerY = renderPos.y + (size / 2);
        ...
        // --- VIEWPORT CULLING ---
        const size = GRID_SIZE * (t.size || 1);
        if (renderPos.x + size < visibleX || renderPos.x > visibleX + visibleW ||
            renderPos.y + size < visibleY || renderPos.y > visibleY + visibleH) {
          return;
        }
        ...
        if (t.id === currentTurn) {
          ...
          ctx.shadowBlur = (10 + pulse * 14) / vs.scale;
      ...
      if (shapeActive && isShapeDragging && shapeStart && shapeEnd) {
        ...
        switch (shapeType) {
          case 'circle': {
            ...
            const labelRadius = radius + 16 / vs.scale;
            const labelX = shapeStart.x + Math.cos(angle) * labelRadius;
            const labelY = shapeStart.y + Math.sin(angle) * labelRadius;
            drawTextBadge(ctx, `${distanceFt} ft radius`, labelX, labelY, vs.scale);
            drawAnchor(ctx, shapeStart.x, shapeStart.y, vs.scale);
            drawAnchor(ctx, shapeEnd.x, shapeEnd.y, vs.scale);
            break;
          }
          case 'rectangle': {
            ...
            const labelY = topY - 16 / vs.scale;
            drawTextBadge(ctx, `${widthFt} ft x ${heightFt} ft`, centerX, labelY, vs.scale);
            drawAnchor(ctx, shapeStart.x, shapeStart.y, vs.scale);
            drawAnchor(ctx, shapeEnd.x, shapeEnd.y, vs.scale);
            break;
          }
          ...
          case 'line': {
            ...
            drawTextBadge(ctx, `${distanceFt} ft line`, midX, midY - 18 / vs.scale, vs.scale);
            drawAnchor(ctx, shapeStart.x, shapeStart.y, vs.scale);
            drawAnchor(ctx, shapeEnd.x, shapeEnd.y, vs.scale);
            break;
          }
      ...
      // Wall drawing preview with selected wallType indicator
      if (role === 'DM' && drawPoints.length > 0) {
        ctx.save();
        ctx.lineWidth = 3 / vs.scale;
        if (wallType === 'door') ctx.strokeStyle = '#f59e0b';
        else if (wallType === 'secret_door') ctx.strokeStyle = '#c084fc';
        else if (wallType === 'window') ctx.strokeStyle = '#38bdf8';
    ...
    if (role === 'DM') {
      notes.forEach(note => {
        const screenX = note.x * vs.scale + vs.x;
        const screenY = note.y * vs.scale + vs.y;
        ctx.save();
  ...
  // FX: Update particle simulation loop
  ...
  useEffect(() => {
    const rawMapSrc = resolveMapUrl(activeMap);
    if (!rawMapSrc) { 
      currentMapSrcRef.current = null;
      initialFitDoneRef.current = false;
      setMapImage(null); 
      return; 
    }

    // Strip query string params (e.g. ?t=123) for map identity comparison
    const cleanMapSrc = rawMapSrc.split('?')[0];
    const previousCleanSrc = (currentMapSrcRef.current || '').split('?')[0];

    // If map URL has not changed, do NOT re-fit or reset viewState
    if (previousCleanSrc === cleanMapSrc && initialFitDoneRef.current) {
      return;
    }

    currentMapSrcRef.current = rawMapSrc;
    const img = new Image();
    ...
    img.onload = () => {
      ...
      // Only perform initial center/fit-to-screen if this map hasn't been fitted yet
      if (!initialFitDoneRef.current || previousCleanSrc !== cleanMapSrc) {
        const canvas = canvasRef.current;
        if (canvas) {
          const clientW = Math.round(canvas.clientWidth) || canvas.width || 1000;
          const clientH = Math.round(canvas.clientHeight) || canvas.height || 800;
          canvas.width = clientW;
          canvas.height = clientH;

          const scale = Math.min(clientW / img.width, clientH / img.height, 1);
          const initialView = { 
            x: (clientW - img.width * scale) / 2, 
            y: (clientH - img.height * scale) / 2, 
            scale 
          };
    ...
    img.onerror = () => {
      console.error("Canvas Engine: Failed to load map image at", img.src);
      currentMapSrcRef.current = null;
      initialFitDoneRef.current = false;
    };
    img.src = rawMapSrc;
  ...
  // Combined High-Performance RAF Loop (Smooth Interpolation + Pings + Flashes)
  useEffect(() => {
    let animId = null;
    let lastTime = performance.now();
    const fpsInterval = 1000 / 60; // 16.67ms target interval for consistent 60 FPS across all refresh rates
    ...
    const loop = (timestamp) => {
      ...
      if (elapsed >= fpsInterval) {
        // Compensate for fractional refresh rate mismatches (144Hz, 165Hz, 240Hz)
        lastTime = timestamp - (elapsed % fpsInterval);
        const dt = fpsInterval / 1000;

        const hasTokenMotion = netInterpolation.update(dt, draggedTokenRef.current?.id);

        // 2. Determine if redraw is required
  ...
  useEffect(() => {
    ...
    const handleStateSync = (state) => {
      ...
      // Synchronize network interpolation buffer
      netInterpolation.syncTokens(mergedTokens);
    ...
    const handleTokenMoved = ({ tokenId, x, y, version }) => {
      ...
      // Pipe to interpolation engine
      netInterpolation.setTarget(tokenId, x, y, false);
    ...
    const handleTokenAdded = ({ token, version }) => {
      if (!token) return;
      netInterpolation.setTarget(token.id, token.x || 0, token.y || 0, true);
      setTokens(prev => {
    ...
    const handleTokenTorchUpdated = ({ tokenId, torchActive, version }) => {
      setTokens(prev => prev.map(token =>
        token.id === tokenId ? { ...token, torchActive: Boolean(torchActive) } : token
      ));
      isFogDirtyRef.current = true;
      if (version) lastStateVersion.current = version;
    };
    ...
    const handleNpcBatchAdded = ({ tokens: newTokens, version }) => {
      ...
      newTokens.forEach(t => netInterpolation.setTarget(t.id, t.x || 0, t.y || 0, true));
      setTokens(prev => {
    ...
    const handleTokenRemoved = ({ tokenId, version }) => {
      netInterpolation.remove(tokenId);
    ...
    const handleTokensDeleted = ({ tokenIds = [], version }) => {
      tokenIds.forEach(id => netInterpolation.remove(id));
    ...
    // ── Interactive Door & Wall Mutators ──
    const handleDoorToggled = ({ wallId, wall, isOpen, isLocked, version }) => {
      setWalls(prev => prev.map(w => {
        if (w.id === wallId) {
          return {
            ...w,
            ...(wall || {}),
            isOpen: isOpen !== undefined ? isOpen : !w.isOpen,
            isLocked: isLocked !== undefined ? isLocked : w.isLocked
          };
        }
        return w;
      }));
    ...
    const handleWallUpdated = ({ wall, version }) => {
      if (!wall) return;
      setWalls(prev => prev.map(w => w.id === wall.id ? { ...w, ...wall } : w));
      isFogDirtyRef.current = true;
      if (version) lastStateVersion.current = version;
    };
    ...
    socket.on('token_torch_updated', handleTokenTorchUpdated);
    ...
    socket.on('door_toggled', handleDoorToggled);
    socket.on('wall_updated', handleWallUpdated);
    ...
    return () => {
      ...
      socket.off('token_torch_updated', handleTokenTorchUpdated);
      ...
      socket.off('door_toggled', handleDoorToggled);
      socket.off('wall_updated', handleWallUpdated);
  ...
  const getMapCoords = useCallback((e) => {
    ...
    const vs = viewStateRef.current;
    return { 
      x: (e.clientX - rect.left - vs.x) / vs.scale, 
      y: (e.clientY - rect.top - vs.y) / vs.scale 
    };
  }, []);
  ...
  const getNoteScreenPos = (note) => {
    ...
    const vs = viewStateRef.current;
    return {
      x: note.x * vs.scale + vs.x + rect.left,
      y: note.y * vs.scale + vs.y + rect.top
    };
  };
  ...
  const handleContextMenu = (e) => {
    ...
    // Finalize walls with active wallType metadata
    ...
    const hitToken = tokens.find(t => {
      const renderPos = netInterpolation.getPosition(t.id, t.x, t.y);
      return t.isPlaced && 
        pos.x >= renderPos.x && pos.x <= renderPos.x + GRID_SIZE * (t.size || 1) &&
        pos.y >= renderPos.y && pos.y <= renderPos.y + GRID_SIZE * (t.size || 1);
    });
    ...
    if (hitToken) {
      ...
      if (role !== 'DM' && hitToken.type === 'npc' && !isOwner) {
        ...
        const isVisible = playerTokens.some(pt => {
          ...
          const poly = computeVisibility(pCenter, validWalls, 1200, mapImage?.width || 5000, mapImage?.height || 5000);
  ...
  // Synchronous Wheel Zoom handler
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    ...
    const handleWheelActive = (e) => {
      ...
      const mouseMapX = (mouseX - current.x) / current.scale;
      const mouseMapY = (mouseY - current.y) / current.scale;

      // Smooth exponent-based scaling (supports trackpads and physical wheels)
      const zoomIntensity = e.ctrlKey ? 0.02 : 0.0015;
      const zoomFactor = Math.exp(-e.deltaY * zoomIntensity);
      const newScale = Math.max(0.08, Math.min(8, current.scale * zoomFactor));
  ...
  const handleMouseDown = useCallback((e) => {
    ...
    // ── INTERACTIVE DOOR TOGGLING (Pan / Default Mode) ──
    if (tool === 'pan' && e.button === 0 && !spaceHeld) {
      const doorProximityRadius = 16 / viewStateRef.current.scale;
      let clickedDoor = null;

      for (const w of walls) {
        if (w.wallType === 'door' || w.wallType === 'secret_door') {
          // Secret doors can only be toggled by the DM, or by players if already opened
          if (w.wallType === 'secret_door' && !w.isOpen && role !== 'DM') {
            continue;
          }

          const midX = (w.x1 + w.x2) / 2;
          const midY = (w.y1 + w.y2) / 2;
          const distToHandle = Math.hypot(pos.x - midX, pos.y - midY);
          const distToSegment = getDistanceToSegment(pos.x, pos.y, w.x1, w.y1, w.x2, w.y2);

          if (distToHandle <= doorProximityRadius || distToSegment <= doorProximityRadius * 0.75) {
            clickedDoor = w;
            break;
          }
        }
      ...
      if (clickedDoor) {
        socket.emit('toggle_door', { wallId: clickedDoor.id });
        soundSynthesizer.playUIClick();
        return;
      }
    }
    ...
    if (role === 'DM' && e.button === 0 && tool === 'erase') {
      ...
      if (closestWall && minWallDist < (15 / viewStateRef.current.scale)) {
      ...
      if (closestLight && closestLightDist < 20 / viewStateRef.current.scale) {
  ...
  }, [shapeActive, tool, shapeMode, shapes, isPointOnShape, role, userId, getMapCoords, measureActive, pingActive, fxActive, placingTokenId, previewPoint, placingStamp, stampOriginalSize, stampSize, previewHeight, tokens, notes, walls, lights, stamps, lightRadius, lightColor, spaceHeld]);
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    if (draggedToken) {
      ...
      const nextViewState = { 
        ...
        x: viewStateRef.current.x + e.movementX, 
        y: viewStateRef.current.y + e.movementY 
      };
      viewStateRef.current = nextViewState;
      setViewState(nextViewState);
      if (drawRef.current) drawRef.current();
  ...
  return (
    <div 
      className="relative w-full h-full select-none overflow-hidden bg-[#0b0c10]"
      style={{ touchAction: 'none' }} // Prevents browser mobile/trackpad gestures from hijacking zoom
    >
      {measureActive && (
      ...
      {/* Hidden container for animated video and static stamps */}
          ...
          if (isVideoFormat(s.url)) {
            return (
              <video
                key={s.id || idx}
                ref={(el) => {
                  if (el) videoElementsRef.current.set(s.url, el);
                  else videoElementsRef.current.delete(s.url);
                }}
                src={s.url}
                muted
                loop
