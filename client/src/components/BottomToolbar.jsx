...
export default function BottomToolbar({
  ...
  const toolDescriptions = {
    ...
    draw: 'Draw Walls & Doors (Shortcut: 2) — Left-click coordinates to draw segments, Right-click to finalize',
  ...
  return (
    <>
      ...
      {/* Wall Type Palette (only visible when draw tool is active) */}
      {tool === 'draw' && (
        <div className="flex items-center gap-1 bg-bgCard border border-borderDark rounded px-1.5 h-7 shrink-0 animate-in fade-in duration-100" title="Select Wall Geometry Type">
          {[
            { id: 'normal', label: '🧱 Wall', title: 'Solid Wall (Blocks vision and movement)' },
            { id: 'door', label: '🚪 Door', title: 'Interactive Door (Clickable to open/close)' },
            { id: 'secret_door', label: '🕵️ Secret', title: 'Secret Passage (Hidden from players until opened)' },
            { id: 'window', label: '🪟 Window', title: 'Window (Allows vision, blocks movement)' }
          ].map(w => (
            <button
              key={w.id}
              onClick={() => setWallType(w.id)}
              className={`px-2 py-0.5 text-[9px] font-bold rounded transition-all ${
                wallType === w.id
                  ? 'bg-accentGold text-black font-extrabold shadow-sm'
                  : 'bg-transparent text-textMuted hover:bg-borderDark hover:text-textLight'
              }`}
              title={w.title}
            >
              {w.label}
