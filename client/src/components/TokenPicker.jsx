...
function TokenItem({ filename, onSelect }) {
  const [hasError, setHasError] = useState(false);
  const baseUrl = (SERVER_URL || '').replace(/\/+$/, '');
  const label = filename.replace(/\.[^/.]+$/, '').replace(/[-_]/g, ' ');
  const imageUrl = `${baseUrl}/assets/tokens/${encodeURIComponent(filename)}`;
    ...
    >
      <div className="w-full aspect-square bg-bgPanel flex items-center justify-center p-1">
        {!hasError ? (
          ...
          <div className="w-full h-full rounded bg-bgCard border border-borderDark flex items-center justify-center text-accentGold font-bold text-sm select-none">
            {initial}
          </div>
        )}
      </div>
...
export default function TokenPicker({ onSelect, onClose }) {
  ...
  const [pos, setPos] = useState({ x: Math.max(20, window.innerWidth - 700), y: 100 });
  ...
  return (
    ...
    >
      ...
      <header className="bg-bgCard p-3 flex justify-between items-center cursor-move border-b border-borderDark shrink-0">
        ...
        <button type="button" onClick={onClose} className="text-textMuted hover:text-white text-xs px-2">✕</button>
      </header>
