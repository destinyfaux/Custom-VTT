...
function TrayTokenAvatar({ name, avatarUrl }) {
  ...
  return (
    <span className="text-[10px] text-accentGold font-bold select-none">
      {initial}
    </span>
  );
}
...
export default function TokenTray({ placingTokenId, setPlacingTokenId }) {
  ...
  const createNpc = (e) => {
    e.preventDefault();
    const qty = Math.max(1, parseInt(newNpc.quantity, 10) || 1);
  ...
  const clearPlayerTrayTokens = () => {
    const playerTokenIds = tokens
      .filter(token => !token.isPlaced && (token.type === 'player' || token.ownedByUser))
      .map(token => token.id);
    if (playerTokenIds.length === 0) return;

    const confirmed = window.confirm(
      `WARNING: Permanently clear ${playerTokenIds.length} player token${playerTokenIds.length === 1 ? '' : 's'} and player-owned companion${playerTokenIds.length === 1 ? '' : 's'} from the tray? They will be recreated when players join.`
    );
    if (confirmed) socket.emit('clear_player_tokens');
  };
        ...
        <div className="p-3 border-t border-borderDark animate-in slide-in-from-top-2 duration-200">
          ...
          <div className="flex justify-between items-center mb-3">
            <div className="flex flex-wrap gap-2">
              ...
              <button
                type="button"
                onClick={clearPlayerTrayTokens}
                disabled={!tokens.some(token => !token.isPlaced && (token.type === 'player' || token.ownedByUser))}
                className="px-2 py-1 rounded text-[9px] font-bold bg-red-950 text-red-200 hover:bg-red-800 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                title="Permanently remove unplaced player and player-owned companion tokens"
              >
                CLEAR PLAYERS
              </button>
          ...
          {showAddNpc && (
            <form onSubmit={createNpc} className="bg-bgCard p-3 rounded border border-accentGold mb-4 space-y-3 shadow-xl">
              ...
              <div className="space-y-1">
                ...
                >
                  <div className="w-6 h-6 rounded-full bg-bgCard border border-borderDark overflow-hidden flex items-center justify-center flex-shrink-0 text-accentGold font-bold text-[10px]">
                    {newNpc.avatarUrl ? (
                      <img
                        src={newNpc.avatarUrl}
                        alt="NPC Avatar"
                        className="w-full h-full object-cover"
                        onError={(e) => { e.currentTarget.style.display = 'none'; }}
                      />
                    ) : (
                      newNpc.name ? newNpc.name.charAt(0).toUpperCase() : '🖼️'
                    )}
                  </div>
              ...
              <div className="flex gap-2">
                <div className="w-1/3 flex flex-col gap-0.5">
                  ...
                  <input
                    ...
                    placeholder="HP"
                    value={newNpc.hp}
                    onChange={e => setNewNpc({ ...newNpc, hp: parseInt(e.target.value, 10) || 0 })}
                  />
                </div>
                <div className="w-1/3 flex flex-col gap-0.5">
                  ...
                  <input
                    ...
                    placeholder="AC"
                    value={newNpc.ac}
                    onChange={e => setNewNpc({ ...newNpc, ac: parseInt(e.target.value, 10) || 0 })}
                  />
                </div>
                <div className="w-1/3 flex flex-col gap-0.5">
                  ...
                  <input
                    ...
                    onChange={e => setNewNpc({ ...newNpc, quantity: Math.max(1, parseInt(e.target.value, 10) || 1) })}
                  />
                </div>
              </div>
          ...
          <div className="space-y-1 max-h-60 overflow-y-auto pr-1 scrollbar-hide">
            ...
            {groupedTokens.map(group => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-full bg-bgPanel border border-borderDark overflow-hidden flex-shrink-0 flex items-center justify-center">
                      <TrayTokenAvatar name={group.name} avatarUrl={group.avatarUrl} />
                    </div>
                  ...
                  <div className="flex gap-1">
                    ...
                    {/* Delete button (deletes all items from the stack pool) */}
                    {group.type === 'npc' && !isCurrentTarget && (
                      <button 
                        type="button"
