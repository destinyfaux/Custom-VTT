...
export default function CharacterSheet({ role, tab, externalData = null, targetUserId = null, onUpdate = null }) {
  ...
  const syncPayload = useMemo(() => ({
      ...data,
      vision: data.vision === 1200 ? 120 : (data.vision ?? 120),
      darkvision: data.darkvision ?? srd.races?.[data.race]?.darkvision ?? null,
      ac: liveAC,
      hpMax: liveMaxHP,
  ...
  const handleAvatarFile = async (e) => {
    ...
    if (!['image/png', 'image/jpeg', 'image/webp', 'image/gif'].includes(file.type) || file.size > 5 * 1024 * 1024) {
      alert('Please choose a PNG, JPG, WEBP, or GIF image under 5 MB.');
      e.target.value = '';
      return;
    }
    const reader = new FileReader();
    reader.onload = (evt) => {
      const characterId = data.id || localStorage.getItem('vtt_active_character_id') || `char_${Date.now()}`;
      fetch(`${SERVER_URL}/api/avatars`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('vtt_session_token') || ''}`
        },
        body: JSON.stringify({ characterId, dataUrl: evt.target.result })
      })
        .then((res) => res.json())
        .then((result) => {
          if (!result.success) throw new Error(result.error || 'Avatar upload failed.');
          update({ id: characterId, avatarUrl: result.avatarUrl });
        })
