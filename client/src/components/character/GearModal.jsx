...
function PetAvatar({ name, tokenUrl }) {
    ...
    if (tokenUrl && !hasError) {
        return (
            <img
                ...
                className="w-12 h-12 rounded-full object-cover border border-borderDark shrink-0 bg-bgPanel"
            />
        );
    }
    ...
    return (
        <div className="w-12 h-12 rounded-full bg-bgPanel border border-borderDark flex items-center justify-center text-accentGold font-bold text-lg shrink-0 select-none">
            {initial}
        </div>
    );
}
...
export default function GearModal({ data, update, onClose, role, targetUserId }) {
    ...
    const handleDeployOwned = (itemOrPet) => {
        ...
        const avatarUrl = resolveTokenUrl(name);

        socket.emit('spawn_owned_token', {
            sourceId: itemOrPet.id || null,
            name,
            avatarUrl,
            hp,
            ac,
                                    ...
                                    >
                                        <div className="flex-1 min-w-0 pr-2">
                                            ...
                                            <div className="text-[10px] text-textMuted mt-0.5 truncate">
                                                {activeCategory === 'pets' 
                                                    ? `CR ${item.cr ?? item.challenge_rating ?? '—'} Beast` 
                    ...
                    <div className="w-1/2 flex flex-col bg-[#0b0c10]">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-[#0d0e12]">
                            ...
                            {activeCategory !== 'pets' ? (
                                ...
                                ownedList.map(pet => {
                                    const canDeploy = Boolean(pet.monsterData);
                                    const tokenUrl = resolveTokenUrl(pet.name);

                                    return (
                                        <div key={pet.id} className="bg-bgCard p-3 rounded border border-borderDark flex items-center justify-between gap-2 text-xs">
                                            <PetAvatar name={pet.name} tokenUrl={tokenUrl} />
                                            <div className="flex-1 min-w-0">
